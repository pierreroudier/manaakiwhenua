...
theme_mw_bw <- function(base_family = "Sans", base_size = 11, ticks = FALSE) {
  ...
  # Add bg colour
  ret <- ret + theme(plot.background = element_rect(fill = "#ffffff", colour = NA))
  ...
  # Add X and Y axis
  ret <- ret + theme(axis.line = element_line(colour = "#282829", size = 0.15))
  
  # Setup axis titles
  ret <- ret + theme(axis.title = element_text(colour = "#282829"))
  
  # Axis labels
  ret <- ret + theme(axis.text = element_text(colour = "#282829"))
  ...
  if (!ticks) {
    ...
    ret <- ret + theme(axis.ticks = element_line(colour = "#282829", size = 0.15))
    ret <- ret + theme(axis.ticks.x = element_line(size = 0.15))
    ret <- ret + theme(axis.ticks.y = element_line(size = 0.15))
    ret <- ret + theme(axis.ticks.length = grid::unit(5, "pt"))
  }
  ...
  # Legend text
  ret <- ret + theme(legend.text = element_text(colour = "#282829"))
  ret <- ret + theme(legend.title = element_text(colour = "#282829"))
  
  # Facet labels
  ret <- ret + theme(strip.text = element_text(colour = "#282829"))
  # Facet background
  ret <- ret + theme(strip.background = element_rect(fill = "#282829"))
  
  # Title and subtitle colours
  ret <- ret + theme(
    plot.title = element_text(colour = "#282829"),
    plot.subtitle = element_text(colour = "#282829")
  )
  
  # Caption
  ret <- ret + theme(plot.caption = element_text(colour = "#282829"))
